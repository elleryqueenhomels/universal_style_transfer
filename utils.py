...
def get_training_images(paths, resize_len=512, crop_height=256, crop_width=256):
    images = []
    for path in paths:
...
def save_images(datas, contents_path, styles_path, save_dir):
    ...
    for content_path in contents_path:
        for style_path in styles_path:
            ...
            save_path = join(save_dir, '%s-%s%s' % (content_name, style_name, content_ext))
